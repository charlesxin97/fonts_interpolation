...
from utils_gsl import cuda, grid2gif
from model_sup_changeZdim import Generator_fc
from dataset_Nswap_fonts_interpolation_inference import return_data
...
class Solver(object):
    def __init__(self, args):
        ...
        self.z_size_start_dim = 4
        self.z_font_color_start_dim = 8
        self.z_back_color_start_dim = 12
        self.z_style_start_dim = 20
    ...
    def train(self):
        ...
        while not out:
            for sup_package in self.data_loader:
                ...
                f = open(os.path.join(self.model_save_dir, self.viz_name, 'back_log.txt'), 'a')
                f.writelines([C_back[0] + ',' + str(C_z_4.clone().tolist()[0]).strip('[]') + '\n'])
                f.close()
                unloader = transforms.ToPILImage()
                dir = os.path.join(self.model_save_dir, self.viz_name, 'sample_img_interpolation')
                ...
                inter_images_z = []
                for i in range(10):
                    inter_z_back.append(torch.tensor(A_z_4.data + 0.1 * i * (C_z_4.data - A_z_4.data)))
                    inter_images_z.append(torch.cat((A_z_1, A_z_2, A_z_3, inter_z_back[i], A_z_5), dim=1))
                inter_images_mid = []
                inter_images = []
                for j in range(10):
                    inter_images_mid.append(self.Autoencoder.fc_decoder(inter_images_z[j]).view(inter_images_z[j].shape[0], 256, 8, 8))
