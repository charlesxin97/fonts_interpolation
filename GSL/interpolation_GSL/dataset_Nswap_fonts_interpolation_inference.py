...
class ilab_Nswap_imgfolder(Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        '''refer'''
        # color 10
        self.back_Colors = {'red': (255, 0, 0), 'green': (0, 255, 0), 'blue': (0, 0, 255)}
        self.back_Colors = list(self.back_Colors.keys())
        self.fg_Colors = {'cyan': (0, 255, 255), 'yellow': (255, 255, 0), 'magenta': (255, 0, 255)}
        self.fg_Colors = list(self.fg_Colors.keys())
        # size 3
        self.Sizes = {'small': 80, 'medium': 100, 'large': 120}
        self.Sizes = list(self.Sizes.keys())
        # style nearly over 100
        for roots, dirs, files in os.walk(os.path.join(self.root, 'a', 'small', 'cyan', 'blue')):
            cates = dirs
            break
    ...
    def findN(self, index):
        ...
        C_font_color = random.choice(self.fg_Colors)
        resume_colors = self.fg_Colors.copy()
        resume_colors.remove(C_font_color)
        C_back_color = random.choice(self.back_Colors)
        ...
        temp_font_color = self.fg_Colors.copy()# avoid same font_color with C
        temp_font_color.remove(C_font_color)
        temp_back_colors = self.back_Colors.copy()  # avoid same back_color with C and avoid same color with font
        ...
        return A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path, C_back_color
    def __getitem__(self, index):
        '''there is a big while loop for choose category and training'''
        A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path, C_back_color = self.findN(index)
        ...
        return {'A': A, 'B': B, 'C': C, 'D': D, 'E': E, 'F': F, 'C_back': C_back_color}
...
def return_data(args):
    ...
    if name.lower() == '3dchairs':
        ...
        if args.use_server == True:
            ...
            root = '/home2/fonts_dataset_version2rgb'
